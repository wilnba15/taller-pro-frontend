...
function buildStats(workOrders: WorkOrder[]): Stats {
  ...
  return workOrders.reduce<Stats>(
    (acc, item) => {
...
function buildBars(stats: Stats) {
  const values = [
    ...
    { label: "En proceso", value: stats.activeCount },
    { label: "Finalizadas", value: stats.completedCount },
    { label: "Entregadas", value: stats.deliveredCount },
  ];
  ...
  return values.map((item) => ({
    ...item,
    width: `${Math.max((item.value / maxValue) * 100, item.value > 0 ? 16 : 6)}%`,
  }));
}

export default async function WorkOrdersPage() {
  const [workOrders, clients, vehicles]: [WorkOrder[], Client[], Vehicle[]] = await Promise.all([
    getWorkOrders(),
    getClients(),
    getVehicles(),
  ]);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Control operativo del taller con resumen de estados e ingresos del mes.
            </p>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5 mb-6">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
            ...
            <p className="mt-2 text-3xl font-bold">{stats.activeCount}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
            <p className="text-sm text-slate-400">Finalizadas</p>
            <p className="mt-2 text-3xl font-bold">{stats.completedCount}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
            <p className="text-sm text-slate-400">Entregadas</p>
            <p className="mt-2 text-3xl font-bold">{stats.deliveredCount}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
            <p className="text-sm text-slate-400">Ingresos del mes</p>
            <p className="mt-2 text-3xl font-bold">{formatMoney(stats.monthlyRevenue)}</p>
          </div>
        </section>

        <section className="grid gap-6 xl:grid-cols-[1.1fr,1.9fr]">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
            <h2 className="text-xl font-semibold">Gráfico operativo</h2>
            <p className="text-slate-400 text-sm mt-1">
              Vista rápida del volumen actual por estado.
            </p>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-slate-800/70 text-slate-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3">Problema</th>
                    ...
                    return (
                      <tr key={order.id} className="border-t border-slate-800 align-top">
                        ...
                        <td className="px-4 py-3">
                          <div className="font-medium">{client?.full_name || `Cliente #${order.client_id}`}</div>
                          <div className="text-slate-400 text-xs">{client?.phone || "-"}</div>
                        </td>
                        <td className="px-4 py-3">
                          {vehicle
                            ? `${vehicle.brand} ${vehicle.model} (${vehicle.plate})`
                            : `Vehículo #${order.vehicle_id}`}
                        </td>
                        <td className="px-4 py-3">{order.entry_date}</td>
                        <td className="px-4 py-3">{order.estimated_delivery_date || "-"}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 max-w-[280px]">
                          <div className="line-clamp-2">{order.issue_description}</div>
                        </td>
                        <td className="px-4 py-3 font-semibold">{formatMoney(order.total)}</td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-2 min-w-[140px]">
                            <Link
                              href={`/dashboard/work-orders/${order.id}/edit`}
                              className="rounded-lg bg-blue-600 px-3 py-2 text-center hover:bg-blue-500 transition"
                            >
                              Editar orden
                            </Link>
                          </div>
